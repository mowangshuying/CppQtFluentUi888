...
FluNavigationItem::FluNavigationItem(QWidget *parent /*= nullptr*/)
    : QWidget(parent) {
  setFixedSize(320, 40);
  ...
  QString qss = FluStyleSheetUitls::getQssByFileName(
      "../StyleSheet/FluNavigationItem.qss");
  setStyleSheet(qss);
  ...
  connect(m_arrow, &QPushButton::clicked, [=](bool b) {
    if (m_bDown) {
      ...
      m_arrow->setIcon(
          FluIconUtils::getFluentIcon(FluAwesomeType::ChevronDown));
      setFixedHeight(40);
      m_wrapBottomW->hide();
...
FluNavigationItem::FluNavigationItem(QIcon icon, QString text,
                                     QWidget *parent /*= nullptr*/)
    : FluNavigationItem(parent) {
  m_icon->setIcon(icon);
  m_label->setText(text);
}

FluNavigationItem::FluNavigationItem(QString text,
                                     QWidget *parent /*= nullptr*/)
    : FluNavigationItem(parent) {
  m_icon->hide();
...
FluNavigationView *FluNavigationItem::getParentView() { return m_parentView; }
...
int FluNavigationItem::calcItemW2Height(FluNavigationItem *item) {
  ...
  for (int i = 0; i < item->m_vBottomLayout->count(); i++) {
    auto tmpItem =
        (FluNavigationItem *)item->m_vBottomLayout->itemAt(i)->widget();
    nH += tmpItem->height();
  }
  return nH;
}
...
void FluNavigationItem::adjustItemHeight(FluNavigationItem *item) {
  ...
  item->setFixedHeight(item->m_wrapTopW->height() +
                       item->m_wrapBottomW->height());
